...
// いままで保存してきたものを取得
$select_sth = $dbh->prepare('SELECT * FROM bbs_entries JOIN users ON bbs_entries.user_id = users.id ORDER BY bbs_entries.created_at DESC');
$select_sth->execute();
...
<p><a href="./profile.php?user_id=<?= $_SESSION['login_user_id'] ?>">自分のプロフィール</a></p>
...
<dt style="margin-left: 3em">
投稿者
</dt>
<dd>
<a href="./profile.php?user_id=<?= $entry['id'] ?>"><?= $entry['name'] ?></a>(ID: <?= htmlspecialchars($entry['user_id']) ?>)
</dd>
</div>
<dt>日時</dt>
